...
class Snake:
    ...
    def get_optimal_policy(self, state):
        ...
        if len(actions_with_max_q_value) > 1:
            rewards = [self.estimate_immediate_reward(state, action) for action in actions_with_max_q_value]
    ...
    def estimate_immediate_reward(self, state, action):
        # Simulate the action and estimate the immediate reward
        ...
        immediate_reward = -distance_to_snack  # Negative distance for prefer smaller distances
    ...
    def calc_reward(self, snack, other_snake):
        ...
        if self.check_out_of_board():
            ...
            reward = -1000  # Punish the snake for getting out of the board
            win_other = True
        ...
        if self.head.pos in list(map(lambda z: z.pos, self.body[1:])):
            ...
            reward = -1000  # Punish the snake for hitting itself
            win_other = True
            self.reset((random.randint(3, 18), random.randint(3, 18)))

        if self.head.pos in list(map(lambda z: z.pos, other_snake.body)):
            if self.head.pos != other_snake.head.pos:
                print("Snake collided with the other snake body")
                reward = -1000  # Punish the snake for hitting the other snake
                win_other = True
                self.reset((random.randint(3, 18), random.randint(3, 18)))
                ...
                if len(self.body) > len(other_snake.body):
                    ...
                    reward = -1000  # Punish the snake for hitting the head of the other snake and being shorter
                    win_other = True
                    self.reset((random.randint(3, 18), random.randint(3, 18)))
